/**
 * build_v3.js — 알파코 제안서 렌더러 V3 (안정성 우선 재설계)
 *
 * V2 → V3 핵심 변경:
 *   ❌ V2: <p:txBody> 전체 재구성 → bodyPr regex 버그로 XML 손상 → 파일 열기 불가
 *   ✅ V3: <p:txBody> 구조 절대 건드리지 않음
 *          <a:t> 텍스트 내용만 교체 (XML 구조 100% 보존)
 *          멀티라인(\n) → 기존 <a:p> 단락에 1:1 매핑
 *          단락 부족 시 마지막 단락 복제하여 추가
 */
...
const OUTPUT = path.join(BASE, 'output/dongdaemun_AX_제안서_v3.pptx');

// ── 12슬라이드 PLAN ──────────────────────────────────────────────────
const PLAN = [
  { src:  0, type: 'TITLE'      },  // S01: 표지
  { src:  1, type: 'TOC'        },  // S02: 목차
  { src: 10, type: 'PROBLEM'    },  // S11: 제안 배경
  { src:  8, type: 'PURPOSE'    },  // S09: 3대 교육 목표
  { src: 14, type: 'FLOW'       },  // S15: Backward Design
  { src:  9, type: 'OVERVIEW'   },  // S10: 3과목 개요
  { src: 18, type: 'CURRICULUM' },  // S19: 커리큘럼 표
  { src: 11, type: 'BENCHMARK'  },  // S12: 벤치마킹
  { src: 12, type: 'CARDS'      },  // S13: USP 3대 강점
  { src:  6, type: 'VENDOR'     },  // S07: 수행 실적
  { src: 41, type: 'KPI'        },  // S42: KPI
...
const CONTENT = {
  ...
  // S02: sp[2]=목차아이템(멀티단락), sp[3]=타이틀, sp[5]=서브타이틀
  TOC: {
    shapes: [
  ...
  CURRICULUM: {
    ...
    rows: [
      ...
      ['3차시  13:30~15:30', 'Custom GPT 챗봇 구현',        'GPTs 빌더·지식 파일 등록·퍼소나 설정 / Custom GPTs 빌더 실습',    '공단 민원 전용 Custom GPT 챗봇'],
      ['4차시  15:45~17:30', '챗봇 QA 및 운영 관리',        'QA 테스트·오류 수정·개인정보 유의사항 / 챗봇 QA + 데모 발표',    '챗봇 운영 가이드북 + 최종 배포판'],
      ['▶ 과목 C: AI 데이터 분석 (8시간)', '', '', ''],
      ['1차시  09:00~10:30', '공단 데이터 현황 분석',        '공단 운영 데이터 구조·핵심 지표 선정 / Claude.ai 분석 실습',      '핵심 지표 5종 AI 분석 프롬프트'],
  ...
  VENDOR: {
    ...
    keepCount: 1,
    // col[0]은 data template에서 <a:t> 없는 구조이므로 '' 고정, col[1]부터 데이터
...
// ════════════════════════════════════════════════════════════════════
// XML 유틸리티 — V3: <a:t> 내용만 교체, XML 구조 불변
// ════════════════════════════════════════════════════════════════════
...
/**
 * ★ V3 핵심 함수 ★
 * <p:txBody> 구조를 절대 건드리지 않는다.
 * 단일행: 첫 번째 <a:t>에만 텍스트, 나머지 비움
 * 다중행: 각 <a:p>에 1:1 매핑 (부족 시 마지막 단락 복제)
 */
function replaceShapeTexts(slideXml, replacements) {
  ...
  return slideXml.replace(/(<p:sp\b)([\s\S]*?)(<\/p:sp>)/g, (full, open, body, close) => {
    ...
    if (lines.length === 1) {
      // ── 단일행: 첫 번째 <a:t>만 교체, 나머지 비움 ──────────────
      let done = false;
      const newBody = body.replace(
        /(<a:t(?![a-zA-Z])[^>]*>)([\s\S]*?)(<\/a:t>)/g,
        (m, o, _t, c) => {
          if (!done) { done = true; return `${o}${esc(str)}${c}`; }
          return `${o}${c}`;
        }
      );
    ...
    const pArr     = [...body.matchAll(pPattern)];
    if (pArr.length === 0) return full;

    const lastP  = pArr[pArr.length - 1][0];  // 추가 라인 삽입 템플릿
    let lineIdx  = 0;

    // 기존 단락 → lines 1:1 매핑
    let newBody = body.replace(pPattern, pXml => {
      if (lineIdx < lines.length) {
        ...
        return pXml.replace(
          /(<a:t(?![a-zA-Z])[^>]*>)([\s\S]*?)(<\/a:t>)/g,
          (m, o, _t, c) => {
            if (firstT) { firstT = false; return `${o}${esc(line)}${c}`; }
            return `${o}${c}`;
          }
        );
      }
    ...
    // 라인 수 > 단락 수이면 마지막 단락 복제하여 추가
    if (lineIdx < lines.length) {
      const extras = lines.slice(lineIdx).map(line => {
        let firstT = true;
        return lastP.replace(
          /(<a:t(?![a-zA-Z])[^>]*>)([\s\S]*?)(<\/a:t>)/g,
          (m, o, _t, c) => {
            if (firstT) { firstT = false; return `${o}${esc(line)}${c}`; }
            return `${o}${c}`;
      ...
      newBody = newBody.replace(/<\/p:txBody>/, extras + '</p:txBody>');
    }
...
/**
 * keepCount 이후 텍스트 shape를 모두 비운다.
 * 원본 템플릿 잔여 텍스트(현대제철 등) 완전 제거.
 */
...
function replaceTableRows(slideXml, rows, headerData = null) {
  return slideXml.replace(/(<a:tbl>)([\s\S]*?)(<\/a:tbl>)/, (full, open, body, close) => {
    ...
    const fillRow = (rowXml, rowData) => {
      let ci = 0;
      return rowXml.replace(/<a:tc\b[\s\S]*?<\/a:tc>/g, cellXml => {
        const v = String(rowData[ci++] !== undefined ? rowData[ci - 1] : '');
        let first = true;
        return cellXml.replace(
          /(<a:t(?![a-zA-Z])[^>]*>)([\s\S]*?)(<\/a:t>)/g,
          (m, o, _t, c) => {
            if (first) { first = false; return `${o}${esc(v)}${c}`; }
            return `${o}${c}`;
          }
        );
      });
    ...
    const finalHeader   = headerData ? fillRow(origHeader, headerData) : origHeader;
    const newRows       = rows.map(r => fillRow(dataTemplate, r));
...
/**
 * spTree 끝에 새 텍스트 shape 추가 (원본에 자리 없는 경우).
 */
function addTextShape(slideXml, { x, y, cx, cy, text, szPt = 14, color = '334155', bold = false }) {
  ...
  const paras    = String(text).split('\n').map(line =>
    `<a:p><a:r>` +
    `<a:rPr lang="ko-KR" sz="${szPt * 100}"${boldAttr} dirty="0">` +
    `<a:solidFill><a:srgbClr val="${color}"/></a:solidFill>` +
    `</a:rPr><a:t>${esc(line)}</a:t></a:r></a:p>`
  ).join('');

  const shape =
    `<p:sp>` +
    `<p:nvSpPr>` +
    `<p:cNvPr id="${id}" name="inj_${id}"/>` +
...
async function main() {
  ...
  // presentation.xml sldIdLst 교체 (ZIP 무결성 보장 — 파일 삭제 없음)
  ...
  console.log('\n📋 PowerPoint 확인 사항:');
  console.log('   1. 슬라이드 12장 정상 표시');
  console.log('   2. 현대제철 로고 이미지 → (주)알파코 로고 수동 교체');
  console.log('   3. 각 슬라이드 텍스트·표 내용 확인\n');
}
