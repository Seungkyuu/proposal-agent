# Agent Skill: Batch-Based 3-Phase Pipeline Orchestration & Integrity Pass Rules (V10)

## 1. 개요 (Overview)
본 사양은 B2B 제안서 기획 에이전트의 구동 시간을 최소화하고 기획자의 인지 피로를 차단하기 위해, **Phase 0(제안사 자산 인벤토리 체크)**를 선행한 후 8단계를 **'3대 마일스톤 페이즈(3-Phase Milestone)'**로 통합 관리하며 각 페이즈 종료 시점마다 **'마스터 스킬 자가 검증(Integrity Pass Rules)'**을 가동하는 대화형 오케스트레이션 사양을 정의한다.

---

## 1-1. [PHASE 0] 제안사 자산 인벤토리 체크 (Phase 1 이전 필수 선행)

> **핵심 원칙:** 에이전트는 고객사명을 입력받은 직후, 고객사 리서치(Phase 1)를 시작하기 **전에** 반드시 아래 3개 질문을 순서대로 사용자에게 확인해야 한다. Phase 0을 생략하거나 Phase 1과 동시에 실행하는 것은 **절대 금지**한다. 이는 제안사의 기존 브랜드 자산을 반영하지 않은 채 임의 디자인으로 진행하는 것을 방지하기 위함이다.

| # | 질문 | YES 처리 | NO 처리 |
|---|---|---|---|
| **Q1** | 제안사의 **기존 PPTX 양식 파일**이 있습니까? 있다면 업로드해 주세요. | 업로드 파일 → **Skill 03 역공학 모드 강제 활성화** → `clone_source_layout_idx` 기반 슬라이드 복제 | 기본 디자인 시스템(Skill 12 default)으로 진행. `clone_source_layout_idx: 0` 고정 |
| **Q2** | 제안사 **브랜드 컬러/로고 가이드**가 있습니까? | Skill 02 적용 시 제안사 브랜드 컬러를 `dominant_bg` 기준으로 우선 반영 | 고객사 CI 기반 60-30-10 팔레트 적용 (Skill 02 기본 로직) |
| **Q3** | 참고할 **이전 유사 제안서(레퍼런스 덱)**가 있습니까? 있다면 업로드해 주세요. | Skill 03 §2 LAYER 2 스토리라인 전개 흐름 분석 추가 실행 → 기획 스토리 DNA 이식 | 표준 9슬라이드 구조(Skill 08)로 진행 |

### Phase 0 출력 포맷 (필수 출력)
```
======================================================================
[PHASE 0] 제안사 자산 인벤토리 체크
======================================================================
1. 기존 PPTX 양식 : [업로드됨 → Skill 03 역공학 모드 활성 / 없음 → 기본 양식 적용]
2. 브랜드 컬러 가이드: [확인됨 → 제안사 브랜드 우선 반영 / 없음 → 고객사 CI 기반]
3. 레퍼런스 덱    : [업로드됨 → 스토리라인 DNA 추출 / 없음 → 표준 9슬라이드 구조]
----------------------------------------------------------------------
>> 위 설정 확인 후 Phase 1(고객사 리서치)을 시작합니다.
   (변경 사항 있으면 지금 말씀해 주세요.)
```

---

## 2. 검증 책임 분류 체계 (Audit Responsibility Classification)

각 검증 항목은 실행 주체에 따라 두 유형으로 분류된다. 이 분류는 에이전트가 스스로 판정할 수 있는 항목과 렌더러 실행 후에야 판정 가능한 항목을 명확히 구분하여, 허위 PASS 판정을 방지한다.

| 유형 | 기호 | 판정 주체 | 판정 시점 |
|---|---|---|---|
| **Claude-Verifiable** | `[CV]` | Claude API 에이전트 | JSON 생성 직후 텍스트/구조 분석으로 즉시 판정 |
| **Renderer-Verified** | `[RV]` | python-pptx 렌더러 | PPTX 파일 생성 후 렌더러가 결과를 보고 |

> **규칙:** `[RV]` 항목은 Phase 완료 보고서에서 `[PENDING-RENDER]`로 표기하며, Phase 4(PPTX 렌더링) 완료 후 최종 확정된다. Claude는 `[RV]` 항목을 스스로 `[PASS]`로 판정하지 않는다.

---

## 3. 3대 페이즈별 자가 검증 항목 및 통과 기준 (Compliance Checklist)

에이전트는 각 페이즈 결과를 사용자에게 출력하기 전, 백그라운드에서 아래 항목을 실행하고 결과를 보고서에 명시한다.

### Phase 0 자가 검증 (Vendor Asset Audit)
* **검증 대상 스킬:** `01 §0`
* **판정 시점:** Phase 1 시작 직전

| # | 항목 | 유형 | 판정 방법 |
|---|---|---|---|
| 1 | **[PPTX 양식 확인 질문 실행]** Pre-Flight Q1을 사용자에게 물었는가 | `[CV]` | 에이전트 발화 기록에 Q1 질문 존재 여부 확인. 생략 시 `[FAIL]` — Phase 1 진입 불가 |
| 2 | **[브랜드 가이드 확인 질문 실행]** Pre-Flight Q2를 사용자에게 물었는가 | `[CV]` | 에이전트 발화 기록에 Q2 질문 존재 여부 확인. 생략 시 `[FAIL]` |
| 3 | **[레퍼런스 덱 확인 질문 실행]** Pre-Flight Q3를 사용자에게 물었는가 | `[CV]` | 에이전트 발화 기록에 Q3 질문 존재 여부 확인. 생략 시 `[FAIL]` |
| 4 | **[자산 반영 여부]** 업로드된 파일이 있다면 Skill 03 역공학 모드가 활성화되었는가 | `[CV]` | Q1 YES이면 `clone_source_layout_idx` ≥ 1인 슬라이드 존재 여부 확인. Q1 NO이면 자동 PASS |

### Phase 1 자가 검증 (Discovery & Strategy Audit)
* **검증 대상 스킬:** `01`, `02`, `03`

| # | 항목 | 유형 | 판정 방법 |
|---|---|---|---|
| 1 | **[웹 검색 완료 여부]** 고객사 사전 리서치가 완료되었는가 | `[CV]` | Skill 01 Section 2 실행 결과 존재 여부 확인. Fallback 적용 시 Case 번호 및 사유 필드에 명시 |
| 2 | **[강사 배제 확인]** 생성된 JSON 내 강사 관련 텍스트 소거 | `[CV]` | Skill 01 Section 7의 정규식 `(강사\|instructor\|담당강사\|수석\|MVP\|약력)` 스캔. 탐지 0건이면 PASS |
| 3 | **[브랜드 컬러 유효성]** 추출된 HEX 코드가 채도/명도 필터를 통과했는가 | `[CV]` | HEX → HSL 변환 후 채도(S) ≥ 20%, 명도(L) 15~85% 범위 확인 (Skill 02 §2 기준값) |
| 4 | **[60-30-10 팔레트 매핑]** JSON의 color_palette 3색이 역할별로 정의되었는가 | `[CV]` | `dominant_bg`, `sub_text`, `accent_color` 3필드 모두 비어있지 않으면 PASS |
| 5 | **[피치 스타일 타당성]** 대상 직급과 pitch_style이 정합하는가 | `[CV]` | 대상이 "임원/C-Level"이면 `HEAVY_ACADEMIC`, 실무진이면 `LIGHT_AGILE` 매핑 확인 |
| 6 | **[60-30-10 실제 슬라이드 적용]** | `[RV]` | 렌더러가 슬라이드 생성 후 배경/텍스트/강조색 면적 비율 검증 |

### Phase 2 자가 검증 (Curriculum & Design Audit)
* **검증 대상 스킬:** `04`, `05`, `06`, `07`

| # | 항목 | 유형 | 판정 방법 |
|---|---|---|---|
| 1 | **[교수설계 모델 M_k 타당성]** 최고점 모델이 선택되었는가 | `[CV]` | O, D, A 변수로 M_k 공식 재연산 후 선택된 모델이 max(M_k)와 일치하면 PASS |
| 2 | **[벤치마킹 AS ≥ 85%]** 글로벌 기준 일치율 충족 여부 | `[CV]` | Section 3의 AS 값 확인. 미달 시 Skill 06 Section 3-1 자동 실행 후 재확인 |
| 3 | **[금지 어휘 0건]** 카피라이팅 필터 통과 | `[CV]` | 정규식 `(이해\|파악\|학습\|교육\|습득)` 스캔. 탐지 0건이면 PASS |
| 4 | **[CURRICULUM_TABLE 5열 헤더 일치]** Skill 05 계약 헤더 준수 | `[CV]` | `headers` 배열이 `["차시", "세부 주제", "핵심 학습 내용", "실습 도구 및 액티비티", "최종 산출물"]`과 정확히 일치하면 PASS |

### Phase 3 자가 검증 (USP & Formatting Audit)
* **검증 대상 스킬:** `08`, `12`

| # | 항목 | 유형 | 판정 방법 |
|---|---|---|---|
| 1 | **[JSON 스키마 일치율 100%]** 최종 JSON이 Skill 12 스키마를 준수하는가 | `[CV]` | `required` 필드(`slide_number`, `layout_type`, `top_message`, `content`) 전체 존재 확인. null 값 0건이면 PASS |
| 2 | **[layout_type enum 유효성]** 9개 허용 값 외 사용 없는가 | `[CV]` | 모든 슬라이드의 `layout_type`이 허용 enum 목록 내에 있으면 PASS |
| 3 | **[세만틱 줄바꿈 삽입]** `top_message` 25자 초과 시 `\n` 존재 | `[CV]` | `top_message` 길이 > 25자인 슬라이드에 `\n` 포함 여부 확인 |
| 4 | **[PPTX 네이티브 표 렌더링]** `add_table` 사용 및 마스터 레이아웃 상속 | `[RV]` | 렌더러 빌드 완료 후 python-pptx의 shape type 검증 |
| 5 | **[폰트 위계 30/18/14pt 적용]** | `[RV]` | 렌더러가 각 placeholder의 font_size 설정값 로그 출력으로 검증 |

---

## 4. 페이즈별 보고서 출력 규격 (Integrity Pass Reports)

### PHASE 0+1 완료 출력 포맷
```
======================================================================
[PHASE 0] 제안사 자산 인벤토리 체크 결과
======================================================================
  - PPTX 양식 파일  : {업로드됨 (파일명) → Skill 03 역공학 모드 활성 / 없음 → 기본 양식}
  - 브랜드 컬러 가이드: {확인됨 (요약) / 없음 → 고객사 CI 기반}
  - 레퍼런스 덱     : {업로드됨 (파일명) → 스토리라인 DNA 추출 / 없음 → 표준 구조}

■ [Phase 0 Audit]
  - [CV: PASS/FAIL] PPTX 양식 확인 질문 실행
  - [CV: PASS/FAIL] 브랜드 가이드 확인 질문 실행
  - [CV: PASS/FAIL] 레퍼런스 덱 확인 질문 실행
  - [CV: PASS/FAIL] 업로드 파일 → Skill 03 역공학 모드 활성화 여부
======================================================================
[PHASE 1 완료] 제안의 방향성 및 제안사 얼라인먼트 수립 안
======================================================================
■ [Step 1~3 기획 요약]
  - 고객사명: {고객사명} (Target: {대상}, {수준} 수준)
  - 과정명: {과정명}
  - 웹 리서치: {정상 완료 / Case 1~3 Fallback 적용}
  * [고객사 최신 행보]: {리서치 요약 또는 "사용자 직접 입력"}
  - 브랜드 강조색: {색상명} ({HEX}) / 피치 스타일: {HEAVY_ACADEMIC / LIGHT_AGILE}

■ [Integrity Pass Audit]
  - [CV: PASS/FAIL] 웹 검색 완료 여부
  - [CV: PASS/FAIL] 강사 배제 정규식 스캔 (탐지 {N}건)
  - [CV: PASS/FAIL] 브랜드 HEX 채도/명도 필터 통과
  - [CV: PASS/FAIL] color_palette 3필드 완전 정의
  - [CV: PASS/FAIL] 피치 스타일 ↔ 대상 직급 정합성
  - [RV: PENDING-RENDER] 60-30-10 실제 슬라이드 적용 (Phase 4 후 확정)
----------------------------------------------------------------------
>> 위 기획 배경, 제안사 실적, 브랜드 컬러 매핑안이 적절합니까?
   (승인: Enter / 수정사항 피드백 입력 / 이전으로: 'back' 입력)
```

### PHASE 2 완료 출력 포맷
```
======================================================================
[PHASE 2 완료] 교수설계 모델 및 제안 커리큘럼 상세안
======================================================================
■ [Step 4~5 기획 요약]
  - 채택 모델: {모델명} (M_k = {점수})
  - 커리큘럼: {일차별 요약}
  - AS 검증: {AS값}% ({정상 / Degraded Mode — 사유 기재})
  - 최종 산출물: {대표 Artifact}

■ [Integrity Pass Audit]
  - [CV: PASS/FAIL] 교수설계 M_k 재연산 일치
  - [CV: PASS/FAIL] 벤치마킹 AS ≥ {85 or 75 (Degraded)}%
  - [CV: PASS/FAIL] 금지 어휘 0건
  - [CV: PASS/FAIL] CURRICULUM_TABLE 5열 헤더 일치
----------------------------------------------------------------------
>> 설계된 커리큘럼이 만족스러우십니까?
   (승인: Enter / 수정사항 피드백 입력 / 이전으로: 'back' 입력)
```

### PHASE 3 완료 출력 포맷
```
======================================================================
[PHASE 3 완료] 제안서 특장점 및 최종 JSON 패키지
======================================================================
■ [Step 6~7 기획 요약]
  - 특장점(USP): {USP 3대 카드 요약}
  - 사후 관리: {커크패트릭 평가 + 사후 케어 계획}

■ [Integrity Pass Audit]
  - [CV: PASS/FAIL] JSON 스키마 required 필드 완전성
  - [CV: PASS/FAIL] layout_type enum 유효성
  - [CV: PASS/FAIL] top_message 세만틱 줄바꿈 삽입
  - [RV: PENDING-RENDER] PPTX 네이티브 표 렌더링 (Phase 4 후 확정)
  - [RV: PENDING-RENDER] 폰트 위계 30/18/14pt 적용 (Phase 4 후 확정)
----------------------------------------------------------------------
>> 이 제안 구조로 최종 실물 PPTX 렌더링을 승인하십니까?
   (최종 빌드 시작: Enter / 기획 수정: 피드백 입력 / 이전으로: 'back' 입력)
```

---

## 5. FAIL 처리 및 Self-Correction 루프

* **`[CV: FAIL]` 감지 시:** 에이전트는 해당 페이즈 결과를 출력하지 않고, 위반 항목을 자가 수정(Self-Correction)한 후 해당 항목만 재검증. 3회 재시도 후에도 FAIL이면 사용자에게 수동 확인 요청.
* **`[RV: PENDING-RENDER]` 항목:** PPTX 생성 완료 시 렌더러가 결과 로그를 출력하며, 에이전트는 이를 수신하여 최종 보고서를 업데이트.
* **사용자 수정 피드백 수신 시:** `10_agent_coherence_propagation_rules.md`의 Dependency Map을 발동하여 연관 단계 전체를 재검증.
